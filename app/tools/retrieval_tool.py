...
class RetrieveFromChatArgs(BaseModel):
    query: str = Field(description="What to look for in the team chat history, in plain English.")


def _run(args: RetrieveFromChatArgs, state: SessionState) -> str:
    session_id = f"retrieval:{state.session_id}:{uuid.uuid4().hex[:8]}"
    try:
        raw = run_retrieval(session_id, state.tenant_id, args.query)
        claim_dicts = json.loads(raw)["claims"]
    except Exception:  # noqa: BLE001 - a sub-agent failure degrades to "no results", not a crash
        return f"Chat retrieval failed for '{args.query}'; try rephrasing the query."

    if not claim_dicts:
        return f"No relevant chat claims found for '{args.query}'."
...
retrieve_from_chat_tool = Tool(
    name="retrieve_from_chat",
    description=(
    ...
    args_schema=RetrieveFromChatArgs,
    handler=_run,
)
