...
from app.tools.plan_query_tool import PlanQueryTool
...
ORCHESTRATOR_TOOLS = build_tool_registry(
    [FinalAnswerTool, AskClarificationTool, PlanQueryTool, *SQL_TOOLS, RetrieveFromChatTool, *ENTITY_LINK_TOOLS]
)
...
def handle_message(session_id: str, tenant_id: str, message: str) -> str:
    ...
    # Each new question needs its own fresh plan -- a plan from a prior
    # question shouldn't silently authorize this one's tool calls.
    state.query_plan = None
    state.needs_sql = None
    state.needs_chat = None
